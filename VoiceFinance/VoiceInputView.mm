...
- (id)init
{
    self = [super initWithFrame:CGRectMake(0, SCREEN_HEIGHT-49, 320, 195)];
    if (self) {
        
        [self setBackgroundColor:[UIColor colorWithRed:111.0f/255.0f green:113.0f/255.0f blue:121.0f/255.0f alpha:1.0]];
        
        UIImageView *normalVoice = [[UIImageView alloc] initWithFrame:CGRectMake(110, 22, 100, 100)];
        normalVoice.image = [UIImage imageNamed:@"voiceNormal"];
       
        [self addSubview:normalVoice];
        
        highVoice = [[UIImageView alloc] initWithFrame:CGRectMake(110, 22, 100, 100)];
        highVoice.image = [UIImage imageNamed:@"voiceHigt"];
        highVoice.alpha = 0;
        ...
        [doneBtn setImage:[UIImage imageNamed:@"completeBtn"] forState:UIControlStateNormal];
//        [doneBtn setImage:[UIImage imageNamed:@"completeBtn"] forState:UIControlStateHighlighted];
...
- (void)show{
    [UIView animateWithDuration:.3 animations:^{
        self.frame = CGRectMake(0, SCREEN_HEIGHT-49-195, 320, 195);
 ...
 - (void)hide{
     [UIView animateWithDuration:.3 animations:^{
         self.frame = CGRectMake(0, SCREEN_HEIGHT-49, 320, 195);
     } completion:^(BOOL finished) {
         if (_updateTimer)
         {
...
- (void)stopRecord{
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(voiceInputDidFailure:)]) {
            [_delegate voiceInputDidFailure:@"未知的网络，数据请求失败"];
        }
        NSLog(@"=========%@", error);
    }];
}
...
{
    VoiceInputView *viv = (__bridge VoiceInputView *)inClientData;
    if (inInterruptionState == kAudioSessionBeginInterruption)
    {
        NSLog(@"stopRecord");
        AudioSessionSetActive(FALSE);
        if (viv != nil && viv->recorder->IsRunning())
        { [viv stopRecord];
        }
    }
